...
public class PowerUpManager : MonoBehaviour {
	...
	void Update () {
		...
		if (timer >= 5) 
		{
			Vector3 positionToPop = new Vector3( Random.Range(boxToPopPowerUp.collider.bounds.min.x,boxToPopPowerUp.collider.bounds.max.x),-2, Random.Range(boxToPopPowerUp.collider.bounds.min.z,boxToPopPowerUp.collider.bounds.max.z));
